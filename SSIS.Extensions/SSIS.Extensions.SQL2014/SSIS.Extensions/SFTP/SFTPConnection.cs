            ...
            try
            {
                ...
                using (SftpClient sftp = new SftpClient(this.hostName, this.portNumber, this.userName, this.passWord))
                {
                    ...
                    foreach (SFTPFileInfo sftpFile in fileList)
                    {
                        ...
                        // Remove Local File
                        if (sftpFile.RemoveSource)
                        {
                            this.Log(String.Format("Removing File: [{0}].", sftpFile.LocalPath), LogLevel.Minimal);
                            File.Delete(sftpFile.LocalPath);
                            this.Log(String.Format("Removed File: [{0}].", sftpFile.LocalPath), LogLevel.Verbose);
                        }
                    }
                }
